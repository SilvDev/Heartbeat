...
public void OnPluginStart()
{
	...
	int matches = GameConfGetOffset(hGameData, "OnTakeDamage_Alive_Match");
	int bytes = GameConfGetOffset(hGameData, "OnTakeDamage_Alive_Bytes");

	gOffset = GameConfGetOffset(hGameData, "OnTakeDamage_Alive_Offset");

	gPatch = GameConfGetAddress(hGameData, "CTerrorPlayer::OnTakeDamage_Alive");
	if( !gPatch ) SetFailState("Error finding the 'CTerrorPlayer::OnTakeDamage_Alive' signature.");

	delete hGameData;

	gBytes = new ArrayList();
	...
	// PATCH
	// ====================
	if( LoadFromAddress(gPatch + view_as<Address>(gOffset), NumberType_Int8) == matches )
	{
		for( int i = 0; i < bytes; i++ )
		{
			gBytes.Push(LoadFromAddress(gPatch + view_as<Address>(gOffset + i), NumberType_Int8));
			if( !g_bLeft4Dead2 && bytes > 30 ) // L4D1 Windows needs to be patched in different areas, strange instructions
			{
				if( (i >= 0 && i <= 6) || (i >= 14 && i <= 31) )
					StoreToAddress(gPatch + view_as<Address>(gOffset + i), 0x90, NumberType_Int8);
			}
			else
			{
				StoreToAddress(gPatch + view_as<Address>(gOffset + i), 0x90, NumberType_Int8);
	...
	{
		if( LoadFromAddress(gPatch + view_as<Address>(gOffset), NumberType_Int8) != 0x90 )
		{
			LogError("Heartbeat: CTerrorPlayer::OnTakeDamage_Alive byte mis-match! %02X", LoadFromAddress(gPatch + view_as<Address>(gOffset), NumberType_Int8));
		}
	}
...
{
	int len = gBytes.Length;

	for( int i = 0; i < len; i++ )
	{
		StoreToAddress(gPatch + view_as<Address>(gOffset + i), gBytes.Get(i), NumberType_Int8);
	}
}
...
{
	// Prevent yelling
	if( g_iReviveCount[client] < g_iCvarVocal && GetClientTeam(client) == 2 )
	{
